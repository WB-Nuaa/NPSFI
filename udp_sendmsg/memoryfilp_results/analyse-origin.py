...
def get_instructions():
    ...
    with open(path + "\\fault_summary1.csv", 'r') as f:
        reader = csv.reader(f)
        all_number = [] #各实际运行指令的粗粒度故障分布，即每一位对应五类大故障
        for row in reader:
            #print("111")
            ...
            if addr in execution_addr or has_fault(fault_types):
                all_number.append(instruction_number)
                #print(instruction_number)
                error_instruction[addr] = fault_types   #各指令细粒度故障分布，即每一位的故障类型，对应小故障
        ...
        for instr_types, value in instruction_types.items():
            ...
            for i in range(len(faults)):
                ...
                for j in faults[i]:
                    error_rate[j] += 1
            # print(error_rate)
            # error_rate = [number / sum(error_rate) for number in  error_rate]
            # print(error_rate)
            # instruction_error_rate[instr_types] = error_rate
